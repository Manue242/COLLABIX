...
En l'état, c'est un **prototype de hackathon** : parfait pour une démo ou un
environnement interne/staging, mais **pas prêt pour une vraie production publique**
sans travaux supplémentaires. Principaux points à traiter avant :

- **Traitement asynchrone** : `/process` bloque plusieurs minutes (timeouts proxy,
  worker occupé). Passer à un système de tâches en arrière-plan + polling/websocket.
- **Authentification / autorisation** : les endpoints sont ouverts.
- **CORS** : restreindre `allow_origins` aux domaines réels du front.
- **Limites d'entrée** : taille max de fichier, formats acceptés, rate limiting.
- **Scalabilité** : Whisper + embeddings sont chargés en mémoire dans le process ;
  prévoir plusieurs workers/instances, éventuellement du GPU, et une base vectorielle
  gérée (ChromaDB en fichier local n'est pas idéal en multi-instances).
- **Observabilité** : logs structurés, métriques, gestion d'erreurs, tests.
- **Packaging** : conteneuriser (Docker) l'API + Ollama pour un déploiement reproductible.

## Déploiement Docker (staging)

Tout est conteneurisé : l'API **et** Ollama démarrent avec une seule commande.
Le modèle `llama3.2` est téléchargé automatiquement au premier lancement.

### Prérequis
- **Docker** + **Docker Compose** (Docker Desktop sur Windows/Mac).
...
Ce qui se passe :
1. Build de l'image de l'API (ffmpeg + dépendances Python inclus).
2. Démarrage du serveur **Ollama** (service `ollama`).
3. Le service `ollama-init` attend Ollama puis fait `ollama pull llama3.2`.
4. L'API écoute sur **http://localhost:8080** (Swagger : `/docs`).

> Le premier build est long (téléchargement de PyTorch, etc.) et le premier
> `/process` télécharge les modèles Whisper + embeddings (mis en cache via des
> volumes, donc une seule fois).
...
docker compose exec ollama ollama list   # modèles Ollama présents
docker compose down               # arrêter (les volumes/persistances restent)
docker compose down -v            # arrêter ET tout supprimer (modèles, index...)
```

### Comment ça marche (réseau)
- L'API joint Ollama via la variable `OLLAMA_HOST=http://ollama:11434` (nom du
  service sur le réseau Docker) — pas besoin de `localhost` côté conteneur.
- Persistances via volumes Docker : modèles Ollama, caches Whisper/embeddings,
  base vectorielle ChromaDB (`chroma_db`).

### Déployer sur un serveur de staging
1. Copier le projet (ou cloner le repo) sur le serveur.
2. `docker compose up -d --build`.
3. Exposer le port `8080` (reverse proxy Nginx/Caddy recommandé pour le TLS).
4. Communiquer l'URL publique à l'équipe front (`API_URL`).

> ⚠️ Avant une vraie prod, voir la section **Mise en production** (async, auth,
> restriction CORS, scalabilité).

## Notes pour la démo

- Modèle Whisper `base` = bon compromis ; passer à `tiny` pour aller plus vite
  (`?model_size=tiny`).
- Si Ollama est lent, utiliser `skip_translation=true` / `skip_summary=true` /
  `skip_chapters=true` pour ne montrer qu'une partie en live.
